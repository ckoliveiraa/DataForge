...
FAKER_REGISTRY: dict[str, callable] = {
    ...
    "float":       lambda f, n, min_value=0, max_value=10_000, **kw: [
                       round(f.pyfloat(min_value=float(min_value), max_value=float(max_value)), 2) for _ in range(n)
                   ],
    ...
    "date":        lambda f, n, min_value="-3y", max_value="today", **kw: [
                       f.date_between(
                           start_date=min_value if isinstance(min_value, str) else _date.fromisoformat(str(min_value)),
                           end_date=max_value if isinstance(max_value, str) else _date.fromisoformat(str(max_value)),
